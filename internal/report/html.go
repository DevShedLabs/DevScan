...
func renderHTML(w io.Writer, r *schema.Report, keys []keyscanner.Finding) error {
	...
	if len(keys) > 0 {
		...
		p(`<section>`)
		p(`  <h2 class="section-heading">Exposed Secrets<span class="heading-count">%d</span></h2>`, len(keys))
		for _, sev := range []keyscanner.Severity{keyscanner.SeverityCritical, keyscanner.SeverityMedium} {
			...
			p(`  <h3 class="sev-%s">%s (%d)</h3>`, string(sev), keyscanSeverityHeadingHTML(sev), counts[sev])
			p(`  <table>`)
...
func keyscanSeverityHeadingHTML(s keyscanner.Severity) string {
	switch s {
