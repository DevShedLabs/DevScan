...
func renderMarkdown(w io.Writer, r *schema.Report, keys []keyscanner.Finding) error {
	...
	// Exposed secrets (--include-keys)
	if len(keys) > 0 {
		counts := map[keyscanner.Severity]int{}
		for _, f := range keys {
			counts[f.Severity]++
		}
		p("## Exposed Secrets (%d total)", len(keys))
		p("")
		p("| Severity | Count |")
		p("|---|---|")
		for _, sev := range []keyscanner.Severity{keyscanner.SeverityCritical, keyscanner.SeverityMedium} {
			if n := counts[sev]; n > 0 {
				p("| %s | %d |", string(sev), n)
			}
		}
		...
		for _, sev := range []keyscanner.Severity{keyscanner.SeverityCritical, keyscanner.SeverityMedium} {
			...
			p("### %s", keyscanSeverityHeadingMD(sev))
			p("")
...
func keyscanSeverityHeadingMD(s keyscanner.Severity) string {
	switch s {
